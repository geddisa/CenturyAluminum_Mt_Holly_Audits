...
# Custom CSS styling to strictly mimic the clean corporate grid look in your screenshotsimport streamlit as st
import pandas as pd
from datetime import date
import os
...
# CSS styling to mimic clean spreadsheet layout rows exactly
st.markdown("""
    <style>
...
def parse_excel_history():
    ...
    if "HK Scores" in xls.sheet_names:
        ...
        # Extract row 5 scores dynamically across historical weeks
        for col_idx in range(2, len(df_hk.columns)):
            for target_row in range(len(df_hk)):
                area_val = df_hk.iloc[target_row, 1]
                if pd.notna(area_val) and str(area_val).strip() in ["Maintenance", "Carbon", "Cast House", "Potline", "Environmental"]:
                    score = df_hk.iloc[target_row, col_idx]
                    if pd.notna(score) and isinstance(score, (int, float)):
                        compiled_records.append({
                            "Date": pd.to_datetime("2026-05-04"),
                            "Auditor": "Excel History",
                            "Area": str(area_val).strip(),
                            "Type": "HK Score",
                            "Score": float(score),
                            "Notes": "Imported from HK Scores tab"
                        })
    ...
    if "Safe Obs - Leadership" in xls.sheet_names:
        ...
        for index, row in df_sol.iterrows():
            ...
            area = row.iloc[1]
            if auditor in name_list:
                completions = sum(1 for cell in row.values if str(cell).strip().lower() in ['c', 'x'])
                if completions > 0:
                    compiled_records.append({
                        "Date": pd.to_datetime("2026-06-01"),
                        "Auditor": str(auditor),
                        "Area": "Multiple Operations" if str(area).lower() == "any" else str(area).strip(),
                        "Type": "Safe Observation",
                        "Score": 100.0,
                        "Notes": f"Completed {completions} checks in schedule cycle"
                    })

    return pd.DataFrame(compiled_records)

# Fallback structure logic checks
